...
def plot_bev_with_agent(scene: Scene, agent: AbstractAgent) -> Tuple[plt.Figure, plt.Axes]:
    ...
    gt_traj = human_trajectory.poses[0:8,:3]
    agent_trajectory = agent.compute_trajectory(scene.get_agent_input(),gt_traj)
    for i in range(len(agent_trajectory)):
        frame_idx = scene.scene_metadata.num_history_frames - 1
        fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
        add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
        add_trajectory_to_bev_ax(ax, human_trajectory, TRAJECTORY_CONFIG["human"])
        add_trajectory_to_bev_ax(ax, agent_trajectory[i], TRAJECTORY_CONFIG["agent"])
        configure_bev_ax(ax)
        configure_ax(ax)
...
def plot_bev_without_agent(scene: Scene, trajectory) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
...
def plot_bev_with_agent_with_token(scene: Scene, agent: AbstractAgent, traj) -> Tuple[plt.Figure, plt.Axes]:
    human_trajectory = scene.get_future_trajectory()
    #gt_traj = human_trajectory.poses[0:8,:3]
    #agent_trajectory = agent.compute_trajectory_with_token(scene.get_agent_input(), gt_traj, token)
    '''
    for i in range(len(agent_trajectory)):
        frame_idx = scene.scene_metadata.num_history_frames - 1
        fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    ...
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax(ax, human_trajectory, TRAJECTORY_CONFIG["human"])
    #add_trajectory_to_bev_ax(ax, agent_trajectory[-1], TRAJECTORY_CONFIG["agent"])
    configure_bev_ax(ax)
    configure_ax(ax)
    #plt.savefig("./vis/" + str(token) + ".png")
    return fig, ax
