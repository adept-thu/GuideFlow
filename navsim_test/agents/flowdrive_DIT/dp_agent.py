...
from navsim.agents.gtrs_flow.dp_modelv2 import DPModel
...
def dp_loss_bev(
        features, targets: Dict[str, torch.Tensor], predictions: Dict[str, torch.Tensor],
...
class DPAgent(AbstractAgent):
    ...
    def initialize(self) -> None:
        """Inherited, see superclass."""
        state_dict: Dict[str, Any] = torch.load(self._checkpoint_path, map_location=torch.device("cpu"), weights_only=True)[
            "state_dict"]
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor], tokens: List[str]) -> Dict[str, torch.Tensor]:
        return self.model(features, targets, tokens)
    ...
    def compute_trajectory(self, agent_input: AgentInput) -> Trajectory:
        ...
        key_list = ['camera_feature', 'camera_feature_back', 'status_feature', 'ego_pose', 'ego_velocity', 'ego_acceleration']
        for key in key_list:
            if key in ['camera_feature', 'camera_feature_back']:
                features[key][0] = torch.Tensor(features[key][0]).unsqueeze(0)
                features[key][1] = torch.Tensor(features[key][1]).unsqueeze(0)
            elif key == 'status_feature':
                features[key][0] = torch.Tensor(features[key][0]).unsqueeze(0)
            else:
                features[key] = torch.Tensor(features[key]).unsqueeze(0)

        # forward pass
        with torch.no_grad():
            predictions = self.forward(features,None,None)
            poses = predictions["trajectory"] #.numpy().squeeze(0)
            Trajs = []
    ...
    def compute_loss(
        ...
        return dp_loss_bev(features, targets, predictions, self._config, self.model._trajectory_head)
