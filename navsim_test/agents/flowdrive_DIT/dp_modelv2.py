...
from navsim.agents.gtrs_flow.models import utils as mutils
...
from navsim.agents.gtrs_flow.models.utils import from_flattened_numpy, to_flattened_numpy
    ...
    def forward(self, kv) -> Dict[str, torch.Tensor]:
        ...
        if not self.training:
            if self.use_rk45:
                ...
                model_fn = mutils.get_model_fn(self.transformer_dp, train=False)
                with torch.no_grad():
                    zt = torch.randn(
                        B, 8, 4,
                        dtype=torch.float32,           
                    ...
                    zt_shape = zt.shape
                    def ode_func(t, zt, arg):
                        zt = from_flattened_numpy(zt, zt_shape).to(device).type(torch.float32)
                        vec_t = torch.ones(zt_shape[0], device=device) * t
                        kv = arg['kv']
                        drift = model_fn(zt, vec_t*100, kv)
                        return to_flattened_numpy(drift)
                    solution = integrate.solve_ivp(ode_func, (eps, 1.), to_flattened_numpy(zt), args=(custom_arg,),
                                rtol=1e-5, atol=1e-5, method='RK45')
                    nfe = solution.nfev
                    #import pdb;pdb.set_trace()
                    traj = torch.tensor(solution.y[:, -1]).reshape(zt_shape).to(device).type(torch.float32)
                    traj = cumsum_traj(traj)
                #'''
            else:
                with torch.no_grad():
                    zt = torch.randn(
