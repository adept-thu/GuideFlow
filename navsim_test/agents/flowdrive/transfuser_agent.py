...
class TransfuserAgent(AbstractAgent):
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, tokens = None) -> Dict[str, torch.Tensor]:
        ...
        """
        return self._transfuser_model(features, targets=targets, tokens=tokens)
    
    def forward_with_token(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, token=None) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        #import pdb;pdb.set_trace()
        """
        if len(targets.shape) == 2:
        ...
        targets = targets_dict
        """
        return self._transfuser_model.forward_test(features,targets=targets,token=token)
    ...
    def compute_trajectory(self, agent_input: AgentInput, gt_traj) -> Trajectory:
        ...
        with torch.no_grad():
            ...
            for i in range(len(poses)):
                pose = predictions["trajectory"][i].numpy().squeeze(0)
                Trajs.append(Trajectory(pose))
        #import pdb;pdb.set_trace()
        # extract trajectory
        return Trajs #Trajectory(poses)
        #import pdb;pdb.set_trace()
        # extract trajectory
        #return Trajectory(poses)
    
    
    def compute_trajectory_with_token(self, agent_input: AgentInput, gt_traj, token) -> Trajectory:
        """
        Computes the ego vehicle trajectory.
        :param current_input: Dataclass with agent inputs.
        :return: Trajectory representing the predicted ego's position in future
        ...
        # forward pass
        with torch.no_grad():
            predictions = self.forward_with_token(features,gt_traj, token)
            poses = predictions["trajectory"]#.squeeze(0).numpy()
            Trajs = []
            for i in range(len(poses)):
                pose = predictions["trajectory"][i].numpy().squeeze(0)
