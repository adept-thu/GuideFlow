...
from navsim.agents.gtrs_flow.dp_agent import DPAgent
...
class AgentLightningModule(pl.LightningModule):
    ...
    def _step(self, batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]], logging_prefix: str) -> Tensor:
        ...
        for k, v in loss_dict.items():
            self.log(f"{logging_prefix}/{k}", v, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=32)

        self.log(f"{logging_prefix}/loss", loss, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=32)
        return loss
    ...
    def predict_step_flow_traj(
        ...
        #import pdb;pdb.set_trace()
        with torch.no_grad():
            predictions = self.agent.forward_with_token(features, targets, tokens)
        return predictions
