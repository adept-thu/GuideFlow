...
class HydraTrajHead(nn.Module):
    ...
    def forward(self, bev_feature, status_encoding, interpolated_traj=None, tokens=None) -> Dict[str, torch.Tensor]:
        ...
        scores = (
                ...
                0.1 * result['traffic_light_compliance'].sigmoid().log() +
                0.5 * result['no_at_fault_collisions'].sigmoid().log() +
                1.0 * result['drivable_area_compliance'].sigmoid().log() +
        ...
        #import pdb;pdb.set_trace()
        pkl_path_1 = "/nas/users/perception-users/yuguanyi/playground_2506/code/GTRS/navhard_two_stage.pkl"
        ...
        for token in tokens:
            sub_score = (
                0.01 * torch.from_numpy(dense_data[token]['imi']).softmax(-1).log() +
                0.1 * torch.from_numpy(dense_data[token]['traffic_light_compliance']).sigmoid().log() +
                0.5 * torch.from_numpy(dense_data[token]['no_at_fault_collisions']).sigmoid().log() +
                0.5 * torch.from_numpy(dense_data[token]['drivable_area_compliance']).sigmoid().log() +
                0.5 * torch.from_numpy(dense_data[token]['driving_direction_compliance']).sigmoid().log() +
                3.0 * (7.0 * torch.from_numpy(dense_data[token]['time_to_collision_within_bound']).sigmoid() +
                    5.0 * torch.from_numpy(dense_data[token]['ego_progress']).sigmoid() +
                    2.0 * torch.from_numpy(dense_data[token]['lane_keeping']).sigmoid()
                    ).log())
            dense_score_list.append(sub_score)
        dense_score = torch.stack(dense_score_list)
        #import pdb;pdb.set_trace()
        scores = scores + dense_score.to(scores.device)
