...
class DPModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor],
        ...
        status_feature: torch.Tensor = features["status_feature"][0]#.unsqueeze(0)

        batch_size = status_feature.shape[0]
        #assert (camera_feature[-1].shape[0] == batch_size)

        camera_feature_curr = camera_feature[-1]#.unsqueeze(0)
        tensor = camera_feature_curr[0].detach().cpu()
    
        #import pdb;pdb.set_trace()
        if isinstance(camera_feature_back, list):
            camera_feature_back_curr = camera_feature_back[-1]#.unsqueeze(0)
        else:
