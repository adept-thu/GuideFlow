...
import time
import pickle
class V8TransfuserModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        ...
        #import pdb;pdb.set_trace()
        trajectory = self._trajectory_head(trajectory_query,agents_query, cross_bev_feature, bev_spatial_shape, status_encoding[:, None], targets=targets, cmd_feature = cmd_feature, global_img=None)
    ...
    def forward_test(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, token=None) -> Dict[str, torch.Tensor]:
        """Torch module forward pass."""
        ...
        output: Dict[str, torch.Tensor] = {"bev_semantic_map": bev_semantic_map}
        #import pdb;pdb.set_trace()
        trajectory = self._trajectory_head.forward_test(trajectory_query,agents_query, cross_bev_feature, bev_spatial_shape, status_encoding[:, None], targets=targets, cmd_feature = cmd_feature, global_img=None, token=token)
        output.update(trajectory)
...
class ReflowTrajectoryHead(nn.Module):
    ...
    '''
    def forward_test(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, targets=None, cmd_feature=None, global_img=None, token=None) -> Dict[str, torch.Tensor]:
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                dt = 1.0/100
                #trajs = []
                timesteps = [i for i in range(40, 100)]
                cmd_feature = self.label_emb(cmd_feature)
                for i_step in timesteps:
                    #print(i_step)
                    tim = torch.tensor(i_step, device=cmd_feature.device).expand(zt.shape[0])
                    ...
                    zt = zt + v_pred * dt
                    #trajs.append(self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)))
                output['trajectory'] = self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)) #trajs
        return output
        '''
    def forward_test(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, targets=None, cmd_feature=None, global_img=None, token=None) -> Dict[str, torch.Tensor]:
            ...
            with torch.no_grad():
                ...
                for i_step in timesteps:
                    ...
                    if self.use_guidance_limit:
                        ...
                        v_pred_no_condition = self.model(tim, zt, torch.zeros(cmd_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
                        v_pred = v_pred_no_condition + 2.5 * (v_pred_condition - v_pred_no_condition)
                    zt = zt + (v_pred + 0.11*limited_v_pred) * dt
                    # 0.05 ,0.08 , 0.15, 0.10, 0.12 , 0.11 best
                    #trajs.append(self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)))
