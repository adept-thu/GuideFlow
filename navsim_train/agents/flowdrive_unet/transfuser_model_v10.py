...
from navsim.agents.flowdrive.unet import UNetModelEnergy as UNetModel
...
class V10TransfuserModel(nn.Module):
    ...
    def forward_test(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, tokens=None) -> Dict[str, torch.Tensor]:
        ...
        output: Dict[str, torch.Tensor] = {"bev_semantic_map": bev_semantic_map}
        seg_map = bev_semantic_map.argmax(1).cpu()
        B, H, W = seg_map.shape
        #import pdb;pdb.set_trace()
        # generate cost map
        cost_map = torch.zeros((B, H, W), dtype=torch.float32, device=seg_map.device)
        cost_mask = (seg_map == 1) | (seg_map == 3) | (seg_map == 5) # 找到 arr 中值等于1或3的位置
        cost_map[cost_mask] = 1
        cost_map = cost_map.cpu().numpy()
        cost_map = np.flip(cost_map, axis=1)  # 沿第0轴（高度）翻转
        cost_map = np.flip(cost_map, axis=2)
        # start_time = time.time() 
        #import pdb;pdb.set_trace()
        distance = np.array([ndimage.distance_transform_edt(slice) for slice in cost_map])
        inv_distance = np.array([ndimage.distance_transform_edt(slice) for slice in (1 - cost_map)])
        drivable_area_sdf = distance - inv_distance # [128, 256]
        drivable_area_sdf = np.clip(drivable_area_sdf, -155, 155)
        drivable_area_sdf = drivable_area_sdf / 155.0
        cost_map_4d = torch.tensor(drivable_area_sdf, device=seg_map.device).unsqueeze(1)
        gt_energy = cost_map_4d

        #import pdb;pdb.set_trace()
        trajectory = self._trajectory_head.forward_test(trajectory_query,agents_query, cross_bev_feature, bev_spatial_shape, status_encoding[:, None], targets=targets, global_img=None, tokens=tokens, gt_energy=gt_energy)
        output.update(trajectory)
        agents = self._agent_head(agents_query)
        output.update(agents)
...
class ReflowTrajectoryHead(nn.Module):
    def __init__(self, num_poses: int, plan_anchor_path: str, config: TransfuserConfig):
        ...
        self.model = UNetModel(image_size=(3, 1, 8), in_channels=3, model_channels=32, out_channels=3, num_res_blocks=1, attention_resolutions="16", num_classes=4)
        ...
        if self.use_condition_limit:
            self.anchor_path = "/nas/users/perception-users/yuguanyi/playground_2506/code/GTRS/navhard_two_stage.pkl"
        ...
        self.esdf_encoder = ESDFEncoder(out_channels=64)
        self.pos_eb = LearnedPositionalEncoding(out_channels=64)
        self.num_poposals = 100
    ...
    def forward(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, targets=None, cmd_feature=None, global_img=None) -> Dict[str, torch.Tensor]:
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                for i_step in timesteps:
                    ...
                    zt = zt + v_pred * dt
                    trajs.append(self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)))
                    if i_step == 95:
                        zt = self.norm_odo(targets['trajectory']).permute(0, 2, 1).unsqueeze(2)
                    #zt = zt + v_pred * dt # [1,2,1,8]
                    #zt = zt.permute(0,2,3,1).squeeze(0)
                    #zt = self.denorm_odo(zt)
                    #trajs.append(self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)))
                output['trajectory'] = trajs
            #'''
            end_time = time.time()
            print(f"单次推理耗时: {(end_time - start_time) * 1000:.2f} 毫秒")
        return output
    
    def forward_test(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, targets=None, global_img=None, tokens=None, gt_energy=None) -> Dict[str, torch.Tensor]:
        output = {}
        #import pdb;pdb.set_trace()
        if not self.training:
            if self.use_condition_limit:
                ...
                for i_token in tokens:
                    ...
                    sampled_limited_points = torch.tensor(sampled_limited_points, device=bev_feature.device)
                    sampled_limited_points = sampled_limited_points.unsqueeze(0)
                    sampled_limited_points = self.norm_odo(sampled_limited_points)
                    sampled_limited_points = sampled_limited_points.permute(0, 2, 1).unsqueeze(2)
                    limited_anchors.append(sampled_limited_points)
                limited_anchors = torch.cat(limited_anchors)    
            with torch.no_grad():
                dt = 1.0/100
                timesteps = [i for i in range(0, 100)]
                B, C, H, W = gt_energy.shape
                #import pdb;pdb.set_trace()
                zt = limited_anchors
                zt_noise = torch.randn(B*self.num_poposals,3,1,8,device=zt.device) * 0.2
                #import pdb;pdb.set_trace()
                zt = zt.repeat_interleave(self.num_poposals,dim=0) + zt_noise
                #import pdb;pdb.set_trace()
                ep_score = []
                for i_token in tokens:
                    ep_score.append(1.0)
                ...
                H = H/4
                W = W/4
                gt_energy = gt_energy.to(torch.float32)
                gt_energy = gt_energy.to(bev_feature.device)
                #import pdb;pdb.set_trace()
                energy_feature = self.esdf_encoder(gt_energy) # [64,128,32,64]

                x_coords = torch.arange(0, H) + 0.5 # 生成整数0~32，截取32个点
                x_coords = torch.flip(x_coords, dims=[0])

                y_coords = torch.arange(-H, H) + 0.5  # 或者直接 torch.arange(-32, 33)[:64]
                y_coords = torch.flip(y_coords, dims=[0])

                grid_x, grid_y = torch.meshgrid(x_coords, y_coords, indexing='ij')
                coord_base = torch.cat([grid_x[None], grid_y[None]], dim=0)[None]
                coord_base = coord_base.expand(B, -1, -1, -1).to(energy_feature.device)
                #import pdb;pdb.set_trace()
                bev_pos = self.pos_eb(coord_base)
                energy_feature = torch.cat([energy_feature, bev_pos], dim=1)
                #import pdb;pdb.set_trace()
                #tim = torch.tensor(i_step, device=energy_feature.device).expand(zt.shape[0])
                energy_feature = energy_feature.repeat_interleave(self.num_poposals,dim=0)
                zt = zt.to(energy_feature.device)
                ep_score = ep_score.repeat_interleave(self.num_poposals,dim=0)
                bev_feature = bev_feature.repeat_interleave(self.num_poposals,dim=0)
                agents_query = agents_query.repeat_interleave(self.num_poposals,dim=0)
                ego_query = ego_query.repeat_interleave(self.num_poposals,dim=0)
                for i_step in timesteps:
                    #print(i_step)
                    # expand num_poposals
                    #energy_feature = energy_feature.expand(self.num_poposals,-1,-1,-1)
                    #import pdb;pdb.set_trace()
                    #import pdb;pdb.set_trace()
                    tim = torch.tensor(i_step, device=energy_feature.device).expand(zt.shape[0])
                    if self.use_guidance_limit:
                        if i_step < 10 or i_step > 90: # 10, 90, no condition
                            ep_score = ep_score.to(tim.device)
                            v_pred_no_condition = self.model(tim, zt, energy_feature, bev_feature, agents_query, ego_query, torch.zeros(ep_score.shape).to(zt.device))
                            #v_pred_ep_condition = self.model(tim, zt, torch.zeros(energy_feature.shape).to(zt.device), bev_feature, agents_query, ego_query, ep_score)
                            v_pred = v_pred_no_condition #+ 2.5 * (v_pred_ep_condition - v_pred_no_condition)
                        else:
                            ep_score = ep_score.to(tim.device)
                            #v_pred_ep_condition = self.model(tim, zt, torch.zeros(energy_feature.shape).to(zt.device), bev_feature, agents_query, ego_query, ep_score)
                            v_pred_condition = self.model(tim, zt, energy_feature, bev_feature, agents_query, ego_query, ep_score)
                            v_pred_no_condition = self.model(tim, zt, torch.zeros(energy_feature.shape).to(zt.device), bev_feature, agents_query, ego_query, torch.zeros(ep_score.shape).to(zt.device))
                            v_pred = v_pred_no_condition + 2.5 * (v_pred_condition - v_pred_no_condition) # + 1.0 * (v_pred_ep_condition - v_pred_no_condition)
                    else:
                        pass
                    zt = zt + v_pred * dt
                    #trajs.append(self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)))
                #import pdb;pdb.set_trace()
                output['trajectory'] = self.denorm_odo(zt.permute(0,2,3,1).squeeze(0)).squeeze(1)
        return output


class ESDFEncoder(nn.Module):
    """
    将 ESDF (shape [1, H=128, W=256] 或 [B,1,128,256]) 编码到 [B,128,32,64]
    通过两个 stride=2 的 conv 层把 H:128->64->32, W:256->128->64，再 conv1x1 到 128 ch.
    """
    def __init__(self, out_channels=128):
        super().__init__()
        self.net = nn.Sequential(
            # input channels = 1
            nn.Conv2d(1, 32, kernel_size=3, stride=2, padding=1),   # 128x256 -> 64x128
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),

            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),  # 64x128 -> 32x64
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),

            nn.Conv2d(64, out_channels, kernel_size=1, stride=1)   # -> out_channels x 32 x 64
        )

    def forward(self, esdf):
        # Accept both [1,128,256] (C,H,W) and [B,1,128,256]
        if esdf.dim() == 3:
            # assume (C,H,W) with C==1
            esdf = esdf.unsqueeze(0)          # -> [1,1,H,W]
        elif esdf.dim() == 4:
            # assume (B,1,H,W)
            pass
        else:
            raise ValueError("esdf must be shape [1,128,256] or [B,1,128,256]")
        out = self.net(esdf)  # [B,128,32,64]
        # if original was single sample without batch, we still keep batch dim for consistency
        return out
...
class LearnedPositionalEncoding(nn.Module):
    def __init__(self, out_channels=128):
        super().__init__()
        # 1x1 conv，把 2 通道 (x,y) -> out_channels
        self.proj = nn.Conv2d(2, out_channels, kernel_size=1)

    def forward(self, pos):
        """
        pos: [B, 2, H, W]，其中 pos[:,0] 是 x，pos[:,1] 是 y
