...
class V7TransfuserModel(nn.Module):
    ...
    def forward_test(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, tokens=None) -> Dict[str, torch.Tensor]:
        ...
        output: Dict[str, torch.Tensor] = {"bev_semantic_map": bev_semantic_map}
        #import pdb;pdb.set_trace()
        trajectory = self._trajectory_head.forward_test(trajectory_query,agents_query, cross_bev_feature, bev_spatial_shape, status_encoding[:, None], targets=targets, cmd_feature = cmd_feature, global_img=None, tokens=tokens)
        output.update(trajectory)
        agents = self._agent_head(agents_query)
        output.update(agents)
...
class ReflowTrajectoryHead(nn.Module):
    ...
    def forward_test(self, ego_query, agents_query, bev_feature, bev_spatial_shape, status_encoding, targets=None, cmd_feature=None, global_img=None, tokens=None) -> Dict[str, torch.Tensor]:
        ...
        if self.training:
            ...
            if self.use_condition_limit:
                ...
                for i_token in tokens:
                    ...
                    sampled_limited_points = torch.tensor(sampled_limited_points, device=cmd_feature.device)
                ...
                limited_v_pred = limited_anchors - zt
                zt = zt + limited_v_pred * dt
                '''
            with torch.no_grad():
                #print(i_step)
                dt = 1.0/100
                ...
                for i_step in timesteps:
                    ...
                    zt = zt.to(cmd_feature.device) 
                    if self.use_guidance_limit:
                        if i_step < 10 or i_step >= 88:
                            ep_score = ep_score.to(tim.device)
                            v_pred_no_condition = self.model(tim, zt, torch.zeros(cmd_feature.shape).to(zt.device), bev_feature, agents_query, ego_query, torch.zeros(ep_score.shape).to(zt.device))
                            v_pred_condition = self.model(tim, zt, torch.zeros(cmd_feature.shape).to(zt.device), bev_feature, agents_query, ego_query, ep_score)
                            v_pred = v_pred_no_condition + 2.5 * (v_pred_condition - v_pred_no_condition)
                        else:
                            ep_score = ep_score.to(tim.device)
                            v_pred_condition = self.model(tim, zt, cmd_feature, bev_feature, agents_query, ego_query, ep_score)
                            v_pred_no_condition = self.model(tim, zt, torch.zeros(cmd_feature.shape).to(zt.device), bev_feature, agents_query, ego_query, ep_score)
                            v_pred = v_pred_no_condition + 2.5 * (v_pred_condition - v_pred_no_condition)
                    else:
                        v_pred_condition = self.model(tim, zt, cmd_feature, bev_feature, agents_query, ego_query)
                        v_pred_no_condition = self.model(tim, zt, torch.zeros(cmd_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
