...
from navsim.agents.flowdrive.unet import UNetModel
...
class ReflowTrajectoryHead(nn.Module):
    ...
    def forward(self, ego_query, agents_query, bev_feature,bev_spatial_shape,status_encoding, targets=None,global_img=None) -> Dict[str, torch.Tensor]:
        ...
        #gt_trajectory = gt_trajectory.permute(0, 2, 1).unsqueeze(2)
        #import pdb;pdb.set_trace()
        ...
        if self.use_target:
            ...
            #target_point_feature[0][0][0] = 25.7
            #target_point_feature[0][0][1] = -6.7
            #import pdb;pdb.set_trace()
        ...
        #import pdb;pdb.set_trace() question
        ...
        if self.training:
            ...
            #import pdb;pdb.set_trace()
            #condition_anchor_feature = condition_anchor_feature * self.generate_condition_mask(1, condition_anchor_feature).to(condition_anchor_feature.device)
            v_pred = self.model(tim, zt, condition_anchor_feature, bev_feature, agents_query, ego_query)
            output["trajectory_loss"]  = self.compute_loss(v_pred,target.float())
            with torch.no_grad():
                zt = torch.randn(z0.shape)
                #import pdb;pdb.set_trace()
                dt = 1.0/25
                for i_step in timesteps:
                    print(i_step)
                    tim = torch.tensor(i_step, device=condition_anchor_feature.device).expand(zt.shape[0])
                    zt = zt.to(condition_anchor_feature.device) 
                    #v_pred_condition = self.model(tim*20, zt, condition_anchor_feature, bev_feature, agents_query, ego_query)
                    #v_pred_no_condition = self.model(tim*20, zt, torch.zeros(condition_anchor_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
                    if self.use_guidance_limit:
                        if i_step < 8 or i_step >= 18:
                            v_pred_no_condition = self.model(tim*4, zt, torch.zeros(condition_anchor_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
                            v_pred = v_pred_no_condition
                        else:
                            v_pred_condition = self.model(tim*4, zt, condition_anchor_feature, bev_feature, agents_query, ego_query)
                            v_pred_no_condition = self.model(tim*4, zt, torch.zeros(condition_anchor_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
                            #v_pred_condition = self.model(tim*4, zt, condition_anchor_feature, bev_feature, agents_query, ego_query)
                            #v_pred_no_condition = self.model(tim*4, zt, torch.zeros(condition_anchor_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
                            v_pred = v_pred_no_condition + 2.5 * (v_pred_condition - v_pred_no_condition)
                    else:
                        v_pred_condition = self.model(tim, zt, condition_anchor_feature, bev_feature, agents_query, ego_query)
                        v_pred_no_condition = self.model(tim, zt, torch.zeros(condition_anchor_feature.shape).to(zt.device), bev_feature, agents_query, ego_query)
                        v_pred = v_pred_no_condition + 2.5 * (v_pred_condition - v_pred_no_condition)
