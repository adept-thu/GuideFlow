...
class SimpleDiffusionTransformer(nn.Module):
    ...
    def forward(self,
        ...
        # (B,To,n_emb)
        cond_embeddings = torch.cat([time_emb, cond, cond_anchor_xy, cond_anchor_angle], dim=1) # [12, 66, 256]
...
def diff_traj(traj):
    ...
    x_diff_norm = x_diff / x_diff_range # [12,8,1]
...
class DPHead(nn.Module):
    def __init__(self, num_poses: int, d_ffn: int, d_model: int, vocab_path: str,
        ...
        self.anchor_path = "/nas/users/perception-users/liulin/workspace/DiffusionDrive/assets/fps_navsim_traj_768_3.npy"
        self.anchor = np.load(self.anchor_path)
        self.plan_anchor_encoder = nn.Sequential(
            *linear_relu_ln(256, 1, 1, 256),
            nn.Linear(256, 256),
        )
        self.plan_anchor_encoder_angle = nn.Sequential(
            *linear_relu_ln(256, 1, 1, 16),
            nn.Linear(256, 256),
        )
    ...
    def match_trajectories_fast(self, query_traj, candidate_trajs):
    ...
    def get_dp_loss(self, kv, gt_trajectory):
        ...
        B = kv.shape[0]
        normal_anchor = diff_traj(torch.tensor(self.anchor).to(gt_trajectory.device).float())
        # [768, 8, 4]
        #import pdb;pdb.set_trace()
        ...
        selected_anchor = self.match_trajectories_fast(gt_trajectory, normal_anchor)
        condition_anchor_xy = selected_anchor[:,:,:,:2]
        condition_anchor_angle = selected_anchor[:,:,:,2:]
        traj_pos_embed = gen_sineembed_for_position(condition_anchor_xy,hidden_dim=32)
        traj_pos_embed = traj_pos_embed.flatten(-2)
        traj_pos_embed = traj_pos_embed.float()
        #import pdb;pdb.set_trace()
        condition_anchor_feature_xy = self.plan_anchor_encoder(traj_pos_embed) # 64 1, 128
        # 12 1 128
        condition_anchor_angle = condition_anchor_angle.reshape(B,1,-1)
        condition_anchor_feature_angle = self.plan_anchor_encoder_angle(condition_anchor_angle)
        mask = self.generate_condition_mask(p_drop=0.8, condition_anchor_feature=condition_anchor_feature_angle)
        condition_anchor_feature_xy = condition_anchor_feature_xy * mask
        condition_anchor_feature_angle = condition_anchor_feature_angle * mask
        #import pdb;pdb.set_trace()
        ...
        pred = self.transformer_dp(
            ...
            timesteps, # [12]
            kv, # [12, 65, 256]
            condition_anchor_feature_xy,
            condition_anchor_feature_angle
        )
