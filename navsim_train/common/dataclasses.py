...
class Scene:
    ...
    def from_scene_dict_list(
        ...
        map_api = cls._build_map_api(scene_metadata.map_name) #liulin change

        frames: List[Frame] = []
        for frame_idx in range(len(scene_dict_list)):
            global_ego_status = cls._build_ego_status(scene_dict_list[frame_idx])
            annotations = cls._build_annotations(scene_dict_list[frame_idx]) #liulin change
        ...
        return Scene(scene_metadata=scene_metadata, map_api=map_api, frames=frames) #liulin change
