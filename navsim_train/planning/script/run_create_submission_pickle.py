...
def run_test_evaluation(
    ...
    for token in tqdm(input_loader.tokens_stage_one, desc="Running first stage evaluation"):
        try:
            print(token) #'b861ed84cd8157ce'
            '''
            tensor([[ 1.1219,  0.0843,  0.0972],
                   [ 2.3240,  0.2623,  0.2278],
                   [ 3.6525,  0.7189,  0.3463],
                   [ 5.0349,  1.4224,  0.4805],
                   [ 6.4074,  2.2505,  0.5922],
                   [ 7.8848,  3.3993,  0.7235],
                   [ 9.0771,  4.7052,  0.8616],
                   [10.0283,  6.1333,  0.9938]])
            '''
