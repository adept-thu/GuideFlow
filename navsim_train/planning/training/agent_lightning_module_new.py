...
class AgentLightningModule(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.gt_path = './assets/gt_data.pkl'
    ...
    def predict_step(
        ...
        if isinstance(self.agent, GTRSAgent):
            ...
            #return self.predict_step_ltf_traj(batch, batch_idx)
            return self.predict_step_flow_traj(batch, batch_idx)
            #raise ValueError('unsupported agent')
    
    def predict_step_flow_traj(
            self,
            batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]],
            batch_idx: int
    ):
        features, targets, tokens = batch # [32,3,512,2048], len(2), len(2)
        self.agent.eval()
        #import pdb;pdb.set_trace()
        #all_trajs = []
        #for token in tokens:
        #    all_trajs.append(self.gt_data[token].cpu())
        #all_trajs = torch.stack(all_trajs).unsqueeze(1).numpy()
        #import pdb;pdb.set_trace()
        #'''
        with torch.no_grad():
            predictions = self.agent.forward(features, targets, tokens)
            all_trajs = predictions['trajectory'].cpu().numpy() # [12,1,8,3]
        #'''
        result = {}
        for batch_idx, token in enumerate(tokens):
            #import pdb;pdb.set_trace()
            final_pose = all_trajs[batch_idx][0]
            result[token] = {
                'trajectory': Trajectory(final_pose, TrajectorySampling(time_horizon=4, interval_length=0.5))
            }
        #import pdb;pdb.set_trace()
        return result
            #raise ValueError('unsupported agent')
    
    def predict_step_ltf_traj(
            self,
            batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]],
            batch_idx: int
    ):
        ...
        with torch.no_grad():
            ...
            all_trajs = predictions['trajectory'].cpu().numpy()
        result = {}
        for batch_idx, token in enumerate(tokens):
            #import pdb;pdb.set_trace()
            final_pose = all_trajs[batch_idx]
            result[token] = {
                'trajectory': Trajectory(final_pose, TrajectorySampling(time_horizon=4, interval_length=0.5))
            }
        #import pdb;pdb.set_trace()
    ...
    def predict_step_combined(
        ...
        #import pdb;pdb.set_trace()
        for batch_idx, token in enumerate(tokens):
            proposals = self.dp_preds_2hz[token]['proposals'] # [100, 8, 3]
            ego_state = EgoState.build_from_rear_axle(
                StateSE2(*features['ego_pose'].cpu().numpy()[batch_idx]),
                tire_steering_angle=0.0,
                vehicle_parameters=self.v_params,
        ...
        with torch.no_grad():
            ...
            lane_keeping_all = predictions["lane_keeping"].sigmoid().log().cpu().numpy()
            traffic_light_compliance_all = predictions["traffic_light_compliance"].sigmoid().log().cpu().numpy()
            indices = predictions["selected_indices"].cpu().numpy()
